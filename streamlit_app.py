...
def build_map(df, clinic_coord, clinic_addr, grid_precision=2):
    ...
    for _, row in grouped.iterrows():
        ...
        # Логарифмическая шкала насыщенности: маленькие группы тоже видны,
        # большие — яркие, но не перекрывают всё
        if max_count > min_count:
            norm = (np.log1p(row['count']) - np.log1p(min_count)) / (np.log1p(max_count) - np.log1p(min_count))
        else:
            norm = 0.5
        fill_opacity = 0.08 + norm * 0.72  # от 0.08 до 0.80
        ...
        folium.Circle(
            ...
            radius=600,  # чуть меньше, меньше перекрытий
            color=color,
            weight=0.3,           # очень тонкая обводка
            opacity=0.25,         # полупрозрачная обводка
            fill=True,
            fill_color=color,
