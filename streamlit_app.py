# -*- coding: utf-8 -*-
"""
🏥 КЛИНИКА-АНАЛИЗАТОР — Streamlit Cloud Edition v1.2
Анализ удалённости пациентов от клиники.
"""
...
def show_dashboard(df, agg):
    ...
    fig1 = px.bar(
        nonzero.reset_index(), x='segment', y='patients', color='segment',
        color_discrete_map=COLORS_MAP,
        text=nonzero.apply(lambda r: f"{int(r['patients'])}<br>({r['patients_pct']}%)", axis=1),
        title='Количество пациентов', labels={'patients': 'Человек', 'segment': ''}
    )
    ...
    if len(nonzero_rev) > 0:
        fig2 = px.bar(
            nonzero_rev.reset_index(), x='segment', y='revenue', color='segment',
            color_discrete_map=COLORS_MAP,
            text=nonzero_rev.apply(lambda r: f"{int(r['revenue']):,} ₽<br>({r['revenue_pct']}%)", axis=1),
            title='Сумма оплат', labels={'revenue': 'Руб.', 'segment': ''}
        )
    ...
    with col_pie1:
        ...
        if len(pie_pat) > 0:
            fig3 = px.pie(pie_pat.reset_index(), names='segment', values='patients',
                          color='segment', color_discrete_map=COLORS_MAP,
                          title='Доля пациентов', hole=0.4)
    ...
    with col_pie2:
        ...
        if len(pie_rev) > 0:
            fig4 = px.pie(pie_rev.reset_index(), names='segment', values='revenue',
                          color='segment', color_discrete_map=COLORS_MAP,
                          title='Доля выручки', hole=0.4)
    ...
    df_local = df[(df['segment'] != 'Другие города') & (df['segment'] != 'Нет данных')].copy()
...
def build_map(df, clinic_coord, clinic_addr, grid_precision=2):
    ...
    center = clinic_coord
    m = folium.Map(location=center, zoom_start=11, tiles="OpenStreetMap", prefer_canvas=True)
    folium.Marker(
        location=center,
    ...
    for _, row in grouped.iterrows():
        ...
        if max_count > min_count:
            norm = (np.log1p(row['count']) - np.log1p(min_count)) / (np.log1p(max_count) - np.log1p(min_count))
        else:
            norm = 0.5
        fill_opacity = 0.08 + norm * 0.72
        popup_html = (
...
def show_map(df):
    ...
    if not clinic_coord or not clinic_addr:
    ...
    with st.spinner("🗺 Построение карты..."):
        m, shown = build_map(df, clinic_coord, clinic_addr)
    if m:
        st.info(f"ℹ️ Показано **{shown}** сгруппированных областей из **{total_valid}** адресов. Все сегменты представлены равномерно.")
        st_folium(m, width=1200, height=700, key="patient_map")
        st.caption("💡 Каждый круг — сгруппированная область (~1.1 км). Цвет = сегмент удалённости, насыщенность = плотность пациентов. Нажмите для деталей.")
    else:
...
def main():
    ...
    st.markdown("---")
    st.caption("Клиника-Анализатор v1.2 | Streamlit Cloud | Plotly + Folium")
